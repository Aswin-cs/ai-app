...
export interface GlideOptionObj {
  value: string;
  label: React.ReactNode;
  tag?: string;
}

export type GlideOption = string | GlideOptionObj;

export interface GlideSelectProps {
  options?: GlideOption[];
  value?: string;
  defaultValue?: string;
  onChange?: (value: string, option: GlideOptionObj) => void;
  placeholder?: string;
  showTags?: boolean;
  accentColor?: string;
  surfaceColor?: string;
...
const GAP = 1;
const MENU_GAP = 6;
const DEFAULT_OPTIONS: GlideOption[] = ["One", "Two", "Three"];

const norm = (o: GlideOption): GlideOptionObj =>
  typeof o === "string" ? { value: o, label: o } : o;

const textOf = (it: GlideOptionObj): string =>
  typeof it.label === "string" ? it.label : it.value;

const typeaheadIndex = (items: GlideOptionObj[], from: number, ch: string) => {
