...
const PHASE_DETAILS = [
  ...
  },
  {
    ...
    badgeBg: "bg-red-500/10 text-red-700 border-red-400",
  ...
  },
  {
    ...
    badgeBg: "bg-purple-500/10 text-purple-700 border-purple-400",
...
export default function MemeVibeCheck({
  ...
  if (!isVisible) {
    return (
      <div className="bg-slate-50/80 rounded-2xl border border-slate-200/80 p-3 flex items-center justify-between transition-all">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-xs">
            ✨
          </div>
          <div>
            <span className="text-xs font-semibold text-slate-800 block">Legal Vibe Check</span>
            <span className="text-[10px] text-slate-500">Meme section is hidden</span>
          </div>
        </div>
        <button
          type="button"
          onClick={onToggleVisible}
          className="px-3 py-1 text-xs font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-lg transition-colors flex items-center gap-1"
        >
  ...
  return (
    <>
      <div className="bg-gradient-to-br from-white via-indigo-50/30 to-purple-50/20 rounded-2xl border border-slate-200/90 shadow-[0_4px_20px_rgba(15,23,42,0.04)] overflow-hidden transition-all duration-300 group">
        {/* Card Header */}
        <div className="px-4 py-3 border-b border-slate-100 bg-white/80 backdrop-blur-md flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-xs font-bold text-[#0F172A] tracking-tight flex items-center gap-1.5">
                Legal Vibe Check
                {simulatedScore !== null ? (
                  <span className="text-[10px] font-mono font-bold text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded border border-amber-300 animate-pulse">
                    Simulator Mode
                  </span>
                ) : (
                  <span className="text-[10px] font-mono font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-200/60">
                    AI Meme
                  </span>
                )}
              </h3>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              type="button"
              onClick={onToggleVisible}
              className="p-1 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors flex items-center gap-1 text-[11px] font-medium"
        ...
        {showSimulator && (
          <div className="bg-amber-50/90 border-b border-amber-200/80 p-3.5 space-y-2.5 text-xs animate-fade-in">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-amber-900 font-bold text-[11px] uppercase tracking-wider">
                <span className="material-symbols-outlined text-[15px] text-amber-600">science</span>
              ...
              {simulatedScore !== null && (
                <button
                  ...
                  className="text-[10px] text-amber-800 underline hover:text-amber-950 font-medium"
                >
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center text-[11px] font-mono">
                <span className="text-amber-800">Adjust Score:</span>
                <span className="font-extrabold text-amber-950 bg-amber-200/70 px-2 py-0.5 rounded">
              ...
              <input
                ...
                className="w-full h-2 bg-amber-200 rounded-lg appearance-none cursor-pointer accent-amber-600 focus:outline-none"
              />
            </div>
        ...
        <div className="p-4 space-y-3">
          ...
          {/* Meme Image Frame */}
          <div className="relative rounded-xl overflow-hidden border border-slate-200/90 shadow-sm bg-slate-900 group/img cursor-pointer transition-transform hover:scale-[1.01]" onClick={() => setIsLightboxOpen(true)}>
